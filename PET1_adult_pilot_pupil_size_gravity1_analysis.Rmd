...
#load("workspaces/gravity1_workspace.RData")
```
...
# Gravity - Ver 1
```{r}
data.preprocessed.gravity1 <- data.preprocessed %>%
  filter(experiment == "gravity" & version == "ver1")
```
...
points(data.preprocessed.gravity1$Xgaze, data.preprocessed.gravity1$Ygaze, pch=16, cex=.5, col=alpha(1), xpd=TRUE)
...
```{r}
dat <- data.preprocessed.gravity1%>%
  rename(time="bin_low")%>%
...
ggsave(pg_s1,file="graphs/gravity1/gravity1_pupil_size_sup_info.png", width=15, height=5, scale=0.7)
...
```{r}
summary_g1<-summary(gam_g1)
summary_g1
sink("saves/gravity1_gam.txt")
print(summary_g1)
sink() 
```
...
ggsave(plot_diff_gamm01, filename = "graphs/gravity1/gravity1_GAMM_diffcurve.png", width=6, height=5, scale=0.7)
...
png("graphs/gravity1/gravity1_GAMM_partial_effect", width=25, height=20, units="cm", res=600)
...
* Summed effects
```{r}
png("graphs/gravity1/gravity1_summed_effect.png", width=14, height=10, units="cm", res=600)
...
```{r}
save.image(file = "workspaces/gravity1_workspace.RData")
```
...
aoi_data_gravity1 <- aoi_data %>%
  filter(experiment == "gravity" & version == "ver1" & IP_LABEL == "IP_gravity1") %>%
...
```{r}
min(aoi_data_gravity1$dwell_time_allAoI)
max(aoi_data_gravity1$dwell_time_allAoI)

# Run the paired t-test
t_test_result <- t.test(dwell_time_allAoI ~ condition,
                        data = aoi_data_gravity1,
                        paired = TRUE)
...
mean_test <- mean(aoi_data_gravity1$dwell_time_allAoI[aoi_data_gravity1$condition == "test"])
mean_cont <- mean(aoi_data_gravity1$dwell_time_allAoI[aoi_data_gravity1$condition == "cont"])


aoi_data_gravity1$condition2 <- jitter(as.numeric(as.factor(aoi_data_gravity1$condition), amount = .0001))

library(gghalves)
y_lim_min = 0
y_lim_max = 10500

gravity1_plot_dwell_end_IP_all_IA <- ggplot(data = aoi_data_gravity1, aes(x = condition, y = dwell_time_allAoI)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_gravity1 %>% filter(condition == "test"),
    aes(x = condition2),
    color = 'darkorange',
    size = 1.5,
    alpha = .5
  ...
  geom_point(
    data = aoi_data_gravity1 %>% filter(condition == "cont"),
    aes(x = condition2),
    color = 'dodgerblue',
    size = 1.5,
    alpha = .5
...
gravity1_plot_dwell_end_IP_all_IA

ggsave(
  gravity1_plot_dwell_end_IP_all_IA,
  filename = "graphs/gravity1/gravity1_dwellTime.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
aoi_data_gravity1 <- aoi_data_gravity1 %>%
...
# Calculate means for the conditions
mean_test <- mean(aoi_data_gravity1$transition_count[aoi_data_gravity1$condition == "test"])
mean_cont <- mean(aoi_data_gravity1$transition_count[aoi_data_gravity1$condition == "cont"])
...
library(gghalves)
y_lim_min = 0
y_lim_max = max(aoi_data_gravity1$transition_count)+3

gravity1_plot_dwell_end_IP_IAtransitioncount <- ggplot(data = aoi_data_gravity1, aes(x = condition, y = transition_count)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_gravity1 %>% filter(condition == "test"),
    aes(x = condition2),
...
gravity1_plot_dwell_end_IP_IAtransitioncount

ggsave(
  gravity1_plot_dwell_end_IP_IAtransitioncount,
  filename = "graphs/gravity1/gravity1_IAtransitions.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
# Read raw text
raw_txt <- readLines("data/sam2_aoi/gravity_ver1_n_cont_sam2.json")
...
sam2_cont_apple <- sam2_cont %>%
  ...
  condition = "cont") %>%
  select(condition, object_label, time_ms, ymin = y_min, ymax = y_max, xmin = x_min, xmax = x_max)


# Read raw text
raw_txt <- readLines("data/sam2_aoi/gravity_ver1_n_test_sam2.json")
# Replace NaN with null
raw_txt <- gsub(": NaN", ": null", raw_txt)
# Parse
...
gaze_y_plot <- ggplot(
  ...
  geom_ribbon(
    data = sam2_cont_apple %>% filter(time_ms < max(dat.agg$time)),
    aes(
      x = time_ms,
      ymin = ymin,
      ymax = ymax,
    ...
    geom_ribbon(
    data = sam2_test_apple %>% filter(time_ms < max(dat.agg$time)),
    aes(
      x = time_ms,
      ymin = ymin,
      ymax = ymax,
  ...
  geom_path(
    ...
    aes(x = time, y = Ygaze, group = RECORDING_SESSION_LABEL),
    size = 0.4,
    alpha = 0.1
  ) +

  # Median path
  geom_path(aes(y = median_y, color = "Median"), size = 1, alpha=0.8) +

  # Mean ± SE ribbon
  geom_ribbon(
    aes(ymin = mean_y - se_y, ymax = mean_y + se_y, fill = "Mean ± SE"),
    alpha = 0.5
  ) +
  ...
  scale_fill_manual(
    values = c("Apple" = "#e88749", "Mean ± SE" = "#2596be"),
    breaks = c("Apple", "Mean ± SE")
  ) 
...
ggsave(
  gaze_y_plot,
  filename = "graphs/gravity1/gravity1_y-gaze.png",
  height = 5,
  width = 8,
  scale = 0.9
)
...
ggsave(
  gaze_x_plot,
  filename = "graphs/gravity1/gravity_ver1_x-gaze.png",
  height = 5,
  width = 10,
  scale = 0.8
)
...
ia_file <- "data/InterestAreas/IA_gravity_ver1.ias"
interest_areas <- read_eyelink_ia(ia_file)

# Load your image and get dimensions
image_path <- "screenshots/gravity_ver1_control.png"  # Updated image path
...
```{r}
library(cowplot)
gravity_ver1_control_photo <- "screenshots/gravity_ver1_control.png"
gravity_ver1_test_photo <- "screenshots/gravity_ver1_test.png"

gravity_ver1_control_photo_gg <- readPNG(gravity_ver1_control_photo)
gravity_ver1_test_photo_gg <- readPNG(gravity_ver1_test_photo)
```


```{r}
# Create the base ggplot with the image as the background
gravity_ver1_control_photo_gg<- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(gravity_ver1_control_photo_gg, width = unit(1, "npc"), height = unit(1, "npc")),
...
# Create the base ggplot with the image as the background
gravity_ver1_test_photo_gg<- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(gravity_ver1_test_photo_gg, width = unit(1, "npc"), height = unit(1, "npc")),
    xmin = 0, xmax = image_width, ymin = 0, ymax = image_height
  ) +
  # Add rectangles for "RECTANGLE" type interest areas
  geom_rect(data = interest_areas %>% filter(Type == "RECTANGLE"),
...
```{r}
gravity1_pics <- plot_grid(
  gravity_ver1_control_photo_gg,
  gravity_ver1_test_photo_gg,
  nrow = 1,
...
```{r}
library(cowplot)
pg1<-plot_grid(agg_data_plot, plot_diff_gamm01, gravity1_plot_dwell_end_IP_all_IA, gravity1_plot_dwell_end_IP_IAtransitioncount, rel_widths = c(2,1, 1, 1),labels=c("C", "D", "E", "F"), nrow=1)

pg2<-plot_grid(gravity1_pics,pg1, ncol=1)

ggsave(pg2, filename = "graphs/gravity1/gravity1_pupil_data.png", width = 14, height=8, scale=0.85)
