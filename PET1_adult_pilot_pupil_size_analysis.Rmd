...
demo_subject_levels <- unique(demo.data$subject)
sample_subject_levels <- unique(sample.data$Session_Name_)
sample_RECORDING_SESSION_LABEL_levels <- unique(sample.data$RECORDING_SESSION_LABEL)

length(unique(sample.data$Session_Name_))
...
sample.data <- sample.data %>%
  ...
  mutate(
    ...
    LEFT_MAD_Threshold = LEFT_median_speed + (8 * LEFT_MAD),
    # Factor 8
    ...
    RIGHT_MAD_Threshold = RIGHT_median_speed + (8 * RIGHT_MAD),
    # Factor 8
...
for (video in unique_videos) {
  ...
  # Create the plot for the current video
  raw_data_plot <- ggplot(data = video_data, aes(x = time.frame, y = LEFT_pupil.inter)) +
    ylab("Pupil size") +
    xlab("Time (in ms)") +
    geom_point(alpha = 1, size = 0.5, color = "darkorange") +
  ...
  # Save the plot
  ggsave(filename = paste0("graphs/", video, "_pupil_blinkcorrected_interpolated.png"), plot = raw_data_plot, width = 18, height = 12)
}
