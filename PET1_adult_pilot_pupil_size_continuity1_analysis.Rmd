...
col2 <- 'black'
col3 <- 'indianred'
#load("workspaces/continuity1_workspace.RData")
```
...
# Continuity - Ver 1
```{r}
data.preprocessed.continuity1 <- data.preprocessed %>%
  filter(experiment == "continuity" & version == "ver1")
```
...
points(data.preprocessed.continuity1$Xgaze, data.preprocessed.continuity1$Ygaze, pch=16, cex=.5, col=alpha(1), xpd=TRUE)
...
```{r}
dat <- data.preprocessed.continuity1%>%
  rename(time="bin_low")%>%
...
ggsave(pg_s1,file="graphs/continuity1/continuity1_pupil_size_sup_info.png", width=15, height=5, scale=0.7)
...
sink("saves/continuity1_gam.txt")
print(summary_g1)
sink()
```
...
ggsave(plot_diff_gamm01, filename = "graphs/continuity1/continuity1_GAMM_diffcurve.png", width=6, height=5, scale=0.7)
...
png("graphs/continuity1/continuity1_GAMM_partial_effect.png", width=25, height=20, units="cm", res=600)
...
* Summed effects
```{r}
png("graphs/continuity1/continuity1_summed_effect.png", width=14, height=10, units="cm", res=600)
...
```{r}
save.image(file = "workspaces/continuity1_workspace.RData")
```
...
aoi_data_continuity1 <- aoi_data %>%
  filter(experiment == "continuity" & version == "ver1" & IP_LABEL == "IP_continuity1") %>%
...
```{r}
min(aoi_data_continuity1$dwell_time_allAoI)
max(aoi_data_continuity1$dwell_time_allAoI)

# Run the paired t-test
t_test_result <- t.test(dwell_time_allAoI ~ condition,
                        data = aoi_data_continuity1,
                        paired = TRUE)
...
mean_test <- mean(aoi_data_continuity1$dwell_time_allAoI[aoi_data_continuity1$condition == "test"])
mean_cont <- mean(aoi_data_continuity1$dwell_time_allAoI[aoi_data_continuity1$condition == "cont"])


aoi_data_continuity1$condition2 <- jitter(as.numeric(as.factor(aoi_data_continuity1$condition), amount = .0001))

library(gghalves)
y_lim_min = 0
y_lim_max = 10500

continuity1_plot_dwell_end_IP_all_IA <- ggplot(data = aoi_data_continuity1, aes(x = condition, y = dwell_time_allAoI)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_continuity1 %>% filter(condition == "test"),
    aes(x = condition2),
    color = 'darkorange',
    size = 1.5,
    alpha = .5
  ...
  geom_point(
    data = aoi_data_continuity1 %>% filter(condition == "cont"),
    aes(x = condition2),
    color = 'dodgerblue',
    size = 1.5,
    alpha = .5
...
continuity1_plot_dwell_end_IP_all_IA

ggsave(
  continuity1_plot_dwell_end_IP_all_IA,
  filename = "graphs/continuity1/continuity1_dwellTime.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
aoi_data_continuity1 <- aoi_data_continuity1 %>%
...
# Calculate means for the conditions
mean_test <- mean(aoi_data_continuity1$transition_count[aoi_data_continuity1$condition == "test"])
mean_cont <- mean(aoi_data_continuity1$transition_count[aoi_data_continuity1$condition == "cont"])
...
library(gghalves)
y_lim_min = 0
y_lim_max = max(aoi_data_continuity1$transition_count)+3

continuity1_plot_dwell_end_IP_IAtransitioncount <- ggplot(data = aoi_data_continuity1, aes(x = condition, y = transition_count)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_continuity1 %>% filter(condition == "test"),
    aes(x = condition2),
...
continuity1_plot_dwell_end_IP_IAtransitioncount

ggsave(
  continuity1_plot_dwell_end_IP_IAtransitioncount,
  filename = "graphs/continuity1/continuity1_IAtransitions.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
# Read raw text
raw_txt <- readLines("data/sam2_aoi/continuity_ver1_n_cont_sam2.json")
...
# Read raw text
raw_txt <- readLines("data/sam2_aoi/continuity_ver1_n_test_sam2.json")
# Replace NaN with null
raw_txt <- gsub(": NaN", ": null", raw_txt)
# Parse
...
sam2_cont_apple_left <- sam2_cont_apple %>% mutate(legend_label = "Left occluder")
sam2_cont_wall_right <- sam2_cont_wall %>% mutate(legend_label = "Right occluder")
...
#   ) +
  ...
  scale_fill_manual(
    values = c("Left occluder" = "#e88749", "Right occluder" = "#633519", "Mean ± SE" = "#2596be"),
    breaks = c("Left occluder", "Right occluder", "Mean ± SE")
  ) 
...
ggsave(
  gaze_x_plot,
  filename = "graphs/continuity1/continuity_ver1_x-gaze.png",
  height = 5,
  width = 10,
  scale = 0.8
)
...
ia_file <- "data/InterestAreas/IA_continuity_ver1.ias"
interest_areas <- read_eyelink_ia(ia_file)

# Load your image and get dimensions
image_path <- "screenshots/continuity_ver1_control.png"  # Updated image path
...
```{r}
library(cowplot)
continuity_ver1_control_photo <- "screenshots/continuity_ver1_control.png"
continuity_ver1_test_photo <- "screenshots/continuity_ver1_test.png"
continuity_ver1_ini_photo <- "screenshots/continuity_ver1_ini.png"



continuity_ver1_ini_photo_gg <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(continuity_ver1_ini_photo)
continuity_ver1_control_photo_gg <- readPNG(continuity_ver1_control_photo)
continuity_ver1_test_photo_gg <- readPNG(continuity_ver1_test_photo)
```


```{r}
# Create the base ggplot with the image as the background
continuity_ver1_control_photo_gg<- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(continuity_ver1_control_photo_gg, width = unit(1, "npc"), height = unit(1, "npc")),
...
# Create the base ggplot with the image as the background
continuity_ver1_test_photo_gg<- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(continuity_ver1_test_photo_gg, width = unit(1, "npc"), height = unit(1, "npc")),
    xmin = 0, xmax = image_width, ymin = 0, ymax = image_height
  ) +
  # Add rectangles for "RECTANGLE" type interest areas
  geom_rect(data = interest_areas %>% filter(Type == "RECTANGLE"),
...
```{r}
continuity1_pics <- plot_grid(continuity_ver1_ini_photo_gg,
  continuity_ver1_control_photo_gg,
  continuity_ver1_test_photo_gg,
  nrow = 1,
  labels = c("A - Initial Position", "B - End Position Control", "C - End Position Test"),
  label_colour = "white"
)
```
...
```{r}
library(cowplot)
pg1<-plot_grid(agg_data_plot, plot_diff_gamm01, continuity1_plot_dwell_end_IP_all_IA,continuity1_plot_dwell_end_IP_IAtransitioncount, rel_widths = c(2,1, 1, 1),labels=c("D", "E", "F", "G"), nrow = 1)

pg2<-plot_grid(continuity1_pics,pg1, ncol=1)

ggsave(pg2, filename = "graphs/continuity1/continuity1_pupil_data.png", width = 14, height=7, scale=0.85)
