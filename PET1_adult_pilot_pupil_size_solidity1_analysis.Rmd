...
#load("workspaces/solidity1_workspace.RData")
...
# Solidity - Ver 1
```{r}
data.preprocessed.solidity1 <- data.preprocessed %>%
  filter(experiment == "solidity" & version == "ver1")
```
...
points(data.preprocessed.solidity1$Xgaze, data.preprocessed.solidity1$Ygaze, pch=16, cex=.5, col=alpha(1), xpd=TRUE)
...
```{r}
dat <- data.preprocessed.solidity1%>%
  rename(time="bin_low")%>%
...
ggsave(pg_s1,file="graphs/solidity1/solidity1_pupil_size_sup_info.png", width=15, height=5, scale=0.7)
...
ggsave(plot_diff_gamm01, filename = "graphs/solidity1/solidity1_GAMM_diffcurve.png", width=6, height=5, scale=0.7)
...
png("graphs/solidity1/solidity1_GAMM_partial_effect.png", width=25, height=20, units="cm", res=600)
...
* Summed effects
```{r}
png("graphs/solidity1/solidity1_summed_effect.png", width=14, height=10, units="cm", res=600)
...
```{r}
save.image(file = "workspaces/solidity1_workspace.RData")
```
...
aoi_data_solidity1 <- aoi_data %>%
  filter(experiment == "solidity" & version == "ver2" & IP_LABEL == "IP_solidity1") %>%
...
```{r}
min(aoi_data_solidity1$dwell_time_allAoI)
max(aoi_data_solidity1$dwell_time_allAoI)

# Run the paired t-test
t_test_result <- t.test(dwell_time_allAoI ~ condition,
                        data = aoi_data_solidity1,
                        paired = TRUE)
...
mean_test <- mean(aoi_data_solidity1$dwell_time_allAoI[aoi_data_solidity1$condition == "test"])
mean_cont <- mean(aoi_data_solidity1$dwell_time_allAoI[aoi_data_solidity1$condition == "cont"])


aoi_data_solidity1$condition2 <- jitter(as.numeric(as.factor(aoi_data_solidity1$condition), amount = .0001))

library(gghalves)
y_lim_min = 0
y_lim_max = 10500

solidity1_plot_dwell_end_IP_all_IA <- ggplot(data = aoi_data_solidity1, aes(x = condition, y = dwell_time_allAoI)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_solidity1 %>% filter(condition == "test"),
    aes(x = condition2),
    color = 'darkorange',
    size = 1.5,
    alpha = .5
  ...
  geom_point(
    data = aoi_data_solidity1 %>% filter(condition == "cont"),
    aes(x = condition2),
    color = 'dodgerblue',
    size = 1.5,
    alpha = .5
...
solidity1_plot_dwell_end_IP_all_IA

ggsave(
  solidity1_plot_dwell_end_IP_all_IA,
  filename = "graphs/solidity1/solidity1_dwellTime.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
aoi_data_solidity1 <- aoi_data_solidity1 %>%
...
# Calculate means for the conditions
mean_test <- mean(aoi_data_solidity1$transition_count[aoi_data_solidity1$condition == "test"])
mean_cont <- mean(aoi_data_solidity1$transition_count[aoi_data_solidity1$condition == "cont"])
...
library(gghalves)
y_lim_min = 0
y_lim_max = max(aoi_data_solidity1$transition_count)+3

solidity1_plot_dwell_end_IP_IAtransitioncount <- ggplot(data = aoi_data_solidity1, aes(x = condition, y = transition_count)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_solidity1 %>% filter(condition == "test"),
    aes(x = condition2),
...
solidity1_plot_dwell_end_IP_IAtransitioncount

ggsave(
  solidity1_plot_dwell_end_IP_IAtransitioncount,
  filename = "graphs/solidity1/solidity1_IAtransitions.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
# Read raw text
raw_txt <- readLines("data/sam2_aoi/solidity_ver1_n_cont_sam2.json")
...
# Now filter apples and compute time
...
# Read raw text
raw_txt <- readLines("data/sam2_aoi/solidity_ver1_n_test_sam2.json")
# Replace NaN with null
raw_txt <- gsub(": NaN", ": null", raw_txt)
# Parse
...
sam2_cont_wall <- sam2_cont_wall %>% mutate(legend_label = "Obstacle")
...
# Obstacle ribbon overlay, with mapped color & fill
  geom_ribbon(
    data = sam2_cont_wall %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
    ...
    geom_ribbon(
    data = sam2_test_wall %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
  ...
  scale_fill_manual(
    values = c("Apple" = "#e88749", "Obstacle" = "#633519", "Mean ± SE" = "#2596be"),
    breaks = c("Apple", "Obstacle", "Mean ± SE")
  ) 
...
ggsave(
  gaze_x_plot,
  filename = "graphs/solidity1/solidity_ver1_x-gaze.png",
  height = 5,
  width = 10,
  scale = 0.8
)
...
ia_file <- "data/InterestAreas/IA_solidity_ver1.ias"
interest_areas <- read_eyelink_ia(ia_file)

# Load your image and get dimensions
image_path <- "screenshots/solidity_ver1_control.png"  # Updated image path
...
```{r}
library(cowplot)
solidity_ver1_control_photo <- "screenshots/solidity_ver1_control.png"
solidity_ver1_test_photo <- "screenshots/solidity_ver1_test.png"
solidity_ver1_ini_photo <- "screenshots/solidity_ver1_ini.png"



solidity_ver1_ini_photo_gg <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(solidity_ver1_ini_photo)
solidity_ver1_control_photo_gg <- readPNG(solidity_ver1_control_photo)
solidity_ver1_test_photo_gg <- readPNG(solidity_ver1_test_photo)
```


```{r}
# Create the base ggplot with the image as the background
solidity_ver1_control_photo_gg<- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(solidity_ver1_control_photo_gg, width = unit(1, "npc"), height = unit(1, "npc")),
...
# Create the base ggplot with the image as the background
solidity_ver1_test_photo_gg<- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(solidity_ver1_test_photo_gg, width = unit(1, "npc"), height = unit(1, "npc")),
    xmin = 0, xmax = image_width, ymin = 0, ymax = image_height
  ) +
  # Add rectangles for "RECTANGLE" type interest areas
  geom_rect(data = interest_areas %>% filter(Type == "RECTANGLE"),
...
```{r}
solidity1_pics <- plot_grid(
  solidity_ver1_ini_photo_gg,
  solidity_ver1_control_photo_gg,
  solidity_ver1_test_photo_gg,
  nrow = 1,
  labels = c("A - before critical event", "B - Control", "C - Test"),
  label_colour = "white"
)
```
...
```{r}
library(cowplot)
pg1<-plot_grid(agg_data_plot, plot_diff_gamm01, solidity1_plot_dwell_end_IP_all_IA, solidity1_plot_dwell_end_IP_IAtransitioncount, rel_widths = c(2,1, 1,1),labels=c("D", "E", "F", "G"), nrow = 1)

pg2<-plot_grid(solidity1_pics,pg1, ncol=1)

ggsave(pg2, filename = "graphs/solidity1/solidity1_pupil_data.png", width = 18, height=8, scale=0.8)
