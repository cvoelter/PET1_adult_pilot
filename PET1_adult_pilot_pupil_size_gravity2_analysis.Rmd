...
col2 <- 'black'
col3 <- 'indianred'
#load("workspaces/gravity2_workspace.RData")
```
...
# Gravity - Ver 2
```{r}
data.preprocessed.gravity2 <- data.preprocessed %>%
  filter(experiment == "gravity" & version == "ver2")
```
...
points(data.preprocessed.gravity2$Xgaze, data.preprocessed.gravity2$Ygaze, pch=16, cex=.5, col=alpha(1), xpd=TRUE)
...
```{r}
dat <- data.preprocessed.gravity2%>%
  rename(time="bin_low")%>%
...
ggsave(pg_s1,file="graphs/gravity2/gravity2_pupil_size_sup_info.png", width=15, height=5, scale=0.7)
...
ggsave(plot_diff_gamm01, filename = "graphs/gravity2/gravity2_GAMM_diffcurve.png", width=6, height=5, scale=0.7)
...
png("graphs/gravity2/gravity2_GAMM_partial_effect.png", width=25, height=20, units="cm", res=600)
...
* Summed effects
```{r}
png("graphs/gravity2/gravity2_summed_effect.png", width=14, height=10, units="cm", res=600)
...
```{r}
save.image(file = "workspaces/gravity2_workspace.RData")
```
...
aoi_data_gravity2 <- aoi_data %>%
  filter(experiment == "gravity" & version == "ver2" & IP_LABEL == "IP_gravity2") %>%
...
```{r}
min(aoi_data_gravity2$dwell_time_allAoI)
max(aoi_data_gravity2$dwell_time_allAoI)

# Run the paired t-test
t_test_result <- t.test(dwell_time_allAoI ~ condition,
                        data = aoi_data_gravity2,
                        paired = TRUE)
...
mean_test <- mean(aoi_data_gravity2$dwell_time_allAoI[aoi_data_gravity2$condition == "test"])
mean_cont <- mean(aoi_data_gravity2$dwell_time_allAoI[aoi_data_gravity2$condition == "cont"])


aoi_data_gravity2$condition2 <- jitter(as.numeric(as.factor(aoi_data_gravity2$condition), amount = .0001))

library(gghalves)
y_lim_min = 0
y_lim_max = 10500

gravity2_plot_dwell_end_IP_all_IA <- ggplot(data = aoi_data_gravity2, aes(x = condition, y = dwell_time_allAoI)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_gravity2 %>% filter(condition == "test"),
    aes(x = condition2),
    color = 'darkorange',
    size = 1.5,
    alpha = .5
  ...
  geom_point(
    data = aoi_data_gravity2 %>% filter(condition == "cont"),
    aes(x = condition2),
    color = 'dodgerblue',
    size = 1.5,
    alpha = .5
...
gravity2_plot_dwell_end_IP_all_IA

ggsave(
  gravity2_plot_dwell_end_IP_all_IA,
  filename = "graphs/gravity2/gravity2_dwellTime.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
aoi_data_gravity2 <- aoi_data_gravity2 %>%
...
# Calculate means for the conditions
mean_test <- mean(aoi_data_gravity2$transition_count[aoi_data_gravity2$condition == "test"])
mean_cont <- mean(aoi_data_gravity2$transition_count[aoi_data_gravity2$condition == "cont"])
...
library(gghalves)
y_lim_min = 0
y_lim_max = max(aoi_data_gravity2$transition_count)+3

gravity2_plot_dwell_end_IP_IAtransitioncount <- ggplot(data = aoi_data_gravity2, aes(x = condition, y = transition_count)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_gravity2 %>% filter(condition == "test"),
    aes(x = condition2),
...
gravity2_plot_dwell_end_IP_IAtransitioncount

ggsave(
  gravity2_plot_dwell_end_IP_IAtransitioncount,
  filename = "graphs/gravity2/gravity2_IAtransitions.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
# Load metadata
sam2_cont <- fromJSON("data/sam2_aoi/gravity_ver2_n_cont_sam2.json")
...
sam2_test <- fromJSON("data/sam2_aoi/gravity_ver2_n_test_sam2.json")
...
gaze_y_plot <- ggplot(
  ...
  geom_ribbon(
    data = sam2_cont_apple %>% filter(time_ms < 13930),
    aes(
      x = time_ms,
      ymin = ymin,
      ymax = ymax,
    ...
    geom_ribbon(
    data = sam2_test_apple %>% filter(time_ms < 13930),
    aes(
      x = time_ms,
      ymin = ymin,
      ymax = ymax,
...
ggsave(
  gaze_y_plot,
  filename = "graphs/gravity2/gravity2_y-gaze.png",
  height = 5,
  width = 8,
  scale = 0.9
)
...
ggsave(
  gaze_x_plot,
  filename = "graphs/gravity2/gravity_ver2_x-gaze.png",
  height = 5,
  width = 10,
  scale = 0.8
)
...
ia_file <- "data/InterestAreas/IA_gravity_ver2.ias"
interest_areas <- read_eyelink_ia(ia_file)

# Load your image and get dimensions
image_path <- "screenshots/gravity_ver2_control.png" # Updated image path
...
```{r}
library(cowplot)
gravity_ver2_control_photo <- "screenshots/gravity_ver2_control.png"
gravity_ver2_test_photo <- "screenshots/gravity_ver2_test.png"

gravity_ver2_control_photo_gg <- readPNG(gravity_ver2_control_photo)
gravity_ver2_test_photo_gg <- readPNG(gravity_ver2_test_photo)
```


```{r}
# Create the base ggplot with the image as the background
gravity_ver2_control_photo_gg <- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(
      gravity_ver2_control_photo_gg,
      width = unit(1, "npc"),
      height = unit(1, "npc")
    ),
    xmin = 0,
  ...
  geom_rect(
    data = interest_areas %>% filter(Type == "RECTANGLE"),
  ...
  geom_ellipse(
    data = interest_areas %>% filter(Type == "ELLIPSE"),
    aes(
      x0 = (X1 + X2) / 2, # Center x
      y0 = (Y1 + Y2) / 2, # Center y
...
# Create the base ggplot with the image as the background
gravity_ver2_test_photo_gg <- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(
      gravity_ver2_test_photo_gg,
      width = unit(1, "npc"),
      height = unit(1, "npc")
    ),
    xmin = 0,
...
```{r}
library(cowplot)
pg1<-plot_grid(agg_data_plot, plot_diff_gamm01, gravity2_plot_dwell_end_IP_all_IA, gravity2_plot_dwell_end_IP_IAtransitioncount, rel_widths = c(2,1,1,1),labels=c("C", "D", "E","F"), nrow = 1)

pg2<-plot_grid(gravity2_pics,pg1, ncol=1)

ggsave(pg2, filename = "graphs/gravity2/gravity2_pupil_data.png", width = 14, height=8, scale=0.85)
