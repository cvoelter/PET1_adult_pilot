...
col2 <- 'black'
col3 <- 'indianred'
#load("workspaces/continuity2_workspace.RData")
```
...
# Continuity - Ver 2
```{r}
data.preprocessed.continuity2 <- data.preprocessed %>%
  filter(experiment == "continuity" & version == "ver2")
```
...
points(data.preprocessed.continuity2$Xgaze, data.preprocessed.continuity2$Ygaze, pch=16, cex=.5, col=alpha(1), xpd=TRUE)
...
```{r}
dat <- data.preprocessed.continuity2%>%
  rename(time="bin_low")%>%
...
ggsave(pg_s1,file="graphs/continuity2/continuity2_pupil_size_sup_info.png", width=15, height=5, scale=0.7)
...
sink("saves/continuity2_gam.txt")
print(summary_g1)
sink() 
```
...
ggsave(plot_diff_gamm01, filename = "graphs/continuity2/continuity2_GAMM_diffcurve.png", width=6, height=5, scale=0.7)
...
png("graphs/continuity2/continuity2_GAMM_partial_effect.png", width=25, height=20, units="cm", res=600)
...
* Summed effects
```{r}
png("graphs/continuity2/continuity2_summed_effect.png", width=14, height=10, units="cm", res=600)
...
```{r}
save.image(file = "workspaces/continuity2_workspace.RData")
```
...
aoi_data_continuity2 <- aoi_data %>%
  filter(experiment == "continuity" & version == "ver2" & IP_LABEL == "IP_continuity2") %>%
...
```{r}
min(aoi_data_continuity2$dwell_time_allAoI)
max(aoi_data_continuity2$dwell_time_allAoI)

# Run the paired t-test
t_test_result <- t.test(dwell_time_allAoI ~ condition,
                        data = aoi_data_continuity2,
                        paired = TRUE)
...
mean_test <- mean(aoi_data_continuity2$dwell_time_allAoI[aoi_data_continuity2$condition == "test"])
mean_cont <- mean(aoi_data_continuity2$dwell_time_allAoI[aoi_data_continuity2$condition == "cont"])


aoi_data_continuity2$condition2 <- jitter(as.numeric(as.factor(aoi_data_continuity2$condition), amount = .0001))

library(gghalves)
y_lim_min = 0
y_lim_max = 10500

continuity2_plot_dwell_end_IP_all_IA <- ggplot(data = aoi_data_continuity2, aes(x = condition, y = dwell_time_allAoI)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_continuity2 %>% filter(condition == "test"),
    aes(x = condition2),
    color = 'darkorange',
    size = 1.5,
    alpha = .5
  ...
  geom_point(
    data = aoi_data_continuity2 %>% filter(condition == "cont"),
    aes(x = condition2),
    color = 'dodgerblue',
    size = 1.5,
    alpha = .5
...
continuity2_plot_dwell_end_IP_all_IA

ggsave(
  continuity2_plot_dwell_end_IP_all_IA,
  filename = "graphs/continuity2/continuity2_dwellTime.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
aoi_data_continuity2 <- aoi_data_continuity2 %>%
...
# Calculate means for the conditions
mean_test <- mean(aoi_data_continuity2$transition_count[aoi_data_continuity2$condition == "test"])
mean_cont <- mean(aoi_data_continuity2$transition_count[aoi_data_continuity2$condition == "cont"])
...
library(gghalves)
y_lim_min = 0
y_lim_max = max(aoi_data_continuity2$transition_count)+3

continuity2_plot_dwell_end_IP_IAtransitioncount <- ggplot(data = aoi_data_continuity2, aes(x = condition, y = transition_count)) +
  #Add geom_() objects
  geom_point(
    data = aoi_data_continuity2 %>% filter(condition == "test"),
    aes(x = condition2),
...
continuity2_plot_dwell_end_IP_IAtransitioncount

ggsave(
  continuity2_plot_dwell_end_IP_IAtransitioncount,
  filename = "graphs/continuity2/continuity2_IAtransitions.png",
  height = 5,
  width = 5,
  scale = 0.8
)
...
### Plotting x gaze coordinates
```{r}
library(jsonlite)
library(dplyr)
(12000/1000)*29.97002997002997

# Read raw text
raw_txt <- readLines("data/sam2_aoi/continuity_ver2_n_cont_sam2.json")
# Replace NaN with null
raw_txt <- gsub(": NaN", ": null", raw_txt)
# Parse
sam2_cont <- fromJSON(paste(raw_txt, collapse = "\n"))


# Unnest bounding_box and center_of_mass into flat columns
sam2_cont <- sam2_cont %>%
  unnest_wider(bounding_box) %>%     # creates x_min, y_min, x_max, y_max
  unnest_wider(center_of_mass)       # creates x, y

# Now filter wall_left and compute time
sam2_cont_wall_left <- sam2_cont %>%
  filter(object_label == "wall_left") %>%
  mutate(time_ms = frame_idx * (1000 / 29.97002997002997),
  condition = "cont") %>%
  select(condition, object_label, time_ms, xmin = x_min, xmax = x_max)

sam2_cont_wall_right <- sam2_cont %>%
  filter(object_label == "wall_right") %>%
  mutate(time_ms = frame_idx * (1000 / 29.97002997002997),
  condition = "cont") %>%
  select(condition, object_label, time_ms, xmin = x_min, xmax = x_max)

sam2_cont_apple <- sam2_cont %>%
  filter(object_label == "apple") %>%
  mutate(time_ms = frame_idx * (1000 / 29.97002997002997),
  condition = "cont") %>%
  select(condition, object_label, time_ms, xmin = x_min, xmax = x_max)

# Read raw text
raw_txt <- readLines("data/sam2_aoi/continuity_ver2_n_test_sam2.json")
# Replace NaN with null
raw_txt <- gsub(": NaN", ": null", raw_txt)
# Parse
sam2_test <- fromJSON(paste(raw_txt, collapse = "\n"))
...
# Now filter wall_left and compute time
sam2_test_wall_left <- sam2_test %>%
  filter(object_label == "wall_left") %>%
  mutate(time_ms = frame_idx * (1000 / 29.97002997002997),
  condition = "test") %>%
  select(condition, object_label, time_ms, xmin = x_min, xmax = x_max)

sam2_test_wall_right <- sam2_test %>%
  filter(object_label == "wall_right") %>%
  mutate(time_ms = frame_idx * (1000 / 29.97002997002997),
  condition = "test") %>%
  select(condition, object_label, time_ms, xmin = x_min, xmax = x_max)

sam2_test_apple <- sam2_test %>%
  filter(object_label == "apple") %>%
  mutate(time_ms = frame_idx * (1000 / 29.97002997002997),
  condition = "test") %>%
  select(condition, object_label, time_ms, xmin = x_min, xmax = x_max)
...
```{r}
dat$condition <- relevel(as.factor(dat$condition), "cont", "test")

dat.agg <- dat %>%
  filter(Xgaze > 0 & Xgaze < 1920) %>%
  group_by(condition, time) %>%
  summarise(
    median_y = median(Ygaze, na.rm = TRUE),
    median_x = median(Xgaze, na.rm = TRUE),
    mean_y = mean(Ygaze, na.rm = TRUE),
    mean_x = mean(Xgaze, na.rm = TRUE),
    se_y = sd(Ygaze, na.rm = TRUE) / sqrt(length(Ygaze)),
    se_x = sd(Xgaze, na.rm = TRUE) / sqrt(length(Xgaze))
  )

con.labs <- c("Control", "Test")
names(con.labs) <- c(levels(as.factor(dat$condition)))
```

```{r}
# Add a label column for the apple data so it can map into the legend
sam2_cont_wall_left <- sam2_cont_wall_left %>% mutate(legend_label = "Left occluder")
sam2_cont_wall_right <- sam2_cont_wall_right %>% mutate(legend_label = "Right occluder")
sam2_cont_apple <- sam2_cont_apple %>% mutate(legend_label = "Apple")

gaze_x_plot <- ggplot(
  data = dat.agg,# %>% filter(time < 14000),
  aes(x = time, y = mean_x)
) +
  facet_wrap(~condition, labeller = labeller(condition = con.labs)) +
    # Apple ribbon overlay, with mapped color & fill
  geom_ribbon(
    data = sam2_cont_apple %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
    ...
    geom_ribbon(
    data = sam2_test_apple %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
  ...
  # Left occluder ribbon overlay, with mapped color & fill
  geom_ribbon(
    data = sam2_cont_wall_left %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
    ...
    geom_ribbon(
    data = sam2_test_wall_left %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
...
# Right occluder ribbon overlay, with mapped color & fill
  geom_ribbon(
    data = sam2_cont_wall_right %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
    ...
    geom_ribbon(
    data = sam2_test_wall_right %>% filter(time_ms < max(dat.agg$time, na.rm=TRUE) & time_ms > min(dat.agg$time, na.rm=TRUE)),
    aes(
      x = time_ms,
      ymin = xmin,
      ymax = xmax,
  ...
  # Raw gaze path
  geom_path(
    data = dat,# %>% filter(time < 14000),
    aes(x = time, y = Xgaze, group = RECORDING_SESSION_LABEL),
    size = 0.4,
    alpha = 0.1
  ) +

  # Median path
  geom_path(aes(y = median_x, color = "Median"), size = 1, alpha=0.8) +

  # Mean ± SE ribbon
  geom_ribbon(
    aes(ymin = mean_x - se_x, ymax = mean_x + se_x, fill = "Mean ± SE"),
    alpha = 0.5
  ) +

  # Mean ± SE points
  geom_point(alpha = 0.9, size = 0.6, color = "#2596be") +
  ...
  theme(
    ...
    legend.key = element_blank(),
    legend.background = element_rect(fill = "transparent"),
    # Reduce space between keys
    legend.spacing.y = unit(0.1, "cm"), # vertical spacing between legend items
    legend.spacing.x = unit(0.1, "cm"), # horizontal spacing between legend items
    
    # Reduce padding inside legend box
    legend.margin = margin(2, 2, 2, 2), # all sides in points
  ...
  # Matching scales for both color and fill
  scale_color_manual(
    values = c(
      "Median" = "black"
    ),
  ...
  scale_fill_manual(
    values = c("Apple" = "#e88749", "Occluder" = "#633519", "Mean ± SE" = "#2596be"),
    breaks = c("Apple", "Occluder", "Mean ± SE")
  ) 
...
ggsave(
  gaze_x_plot,
  filename = "graphs/continuity2/continuity_ver2_x-gaze.png",
  height = 5,
  width = 10,
  scale = 0.8
)
...
dat.agg <- dat %>%
  filter(Xgaze > 0 & Ygaze < 1920) %>%
  group_by(condition, time) %>%
  summarise(median_y = median(Ygaze, na.rm = TRUE),
            median_x = median(Xgaze, na.rm = TRUE),
            mean_y = mean(Ygaze, na.rm = TRUE),
            mean_x = mean(Xgaze, na.rm = TRUE),
            se_y = sd(Ygaze, na.rm = TRUE)/sqrt(length(Ygaze)),
...
names(con.labs) <- c(levels(as.factor(dat$condition)))
# 
dat.agg$x <- rep("x", nrow(dat.agg))
dat.agg$y <- rep("y", nrow(dat.agg))

gaze_x_plot <- ggplot(data = dat.agg, aes(x = time, y = mean_x)) +
  facet_wrap( ~ condition, labeller = labeller(condition = con.labs)) +
  # #moving ball:
  # geom_ribbon(
  #   data = dat %>% filter(condition != "cont") %>% filter(time %in% (9000:14000)),
  #   aes(x = time.frame, ymin = x_l_approx, ymax = x_r_approx),
  #   fill = "#988B2D",
  #   alpha = 0.8
  # ) +
  # geom_ribbon(
  #   data = dat %>% filter(condition == "con") %>% filter(time.frame %in% (9000:14000)),
  #   aes(
  #     x = time.frame,
  #     ymin = x_l_approx,
  #     ymax = x_r_approx,
  ...
  # ) +
  # geom_ribbon(
  #   data = dat %>% filter(condition != "hab") %>% filter(time.frame %in% (4250:8500)),
  #   aes(x = time.frame, ymin = x_l_approx, ymax = x_r_approx),
  #   fill = "#988B2D",
  #   alpha = 0.8
  # ) +
  # geom_ribbon(
  #   data = dat %>% filter(condition == "hab") %>% filter(time.frame < 5200),
  #   aes(x = time.frame, ymin = x_l_approx, ymax = x_r_approx),
  #   fill = "#988B2D",
  #   alpha = 0.8
  # ) +
  # #left occluder
  # geom_ribbon(
  #   data = dat %>% filter(condition != "hab"),
  #   aes(x = time.frame, ymin = 280, ymax = 440),
  #   fill = "#4F6AC1",
  #   alpha = 1
  # ) +
  # #right occluder
  # geom_ribbon(
  #   data = dat %>% filter(condition != "hab"),
  #   aes(x = time.frame, ymin = 580, ymax = 740),
  #   fill = "#4F6AC1",
  #   alpha = 1
  # ) +
  geom_point(alpha = 0.9, size = 0.7, color = "darkgreen") +

 geom_path(data = dat, aes(x= time, y = Xgaze, group = RECORDING_SESSION_LABEL), size = 0.7, alpha=0.1) +
  geom_path(aes(y = median_x), size = 1.2) +
 geom_ribbon(aes(ymin = mean_x - se_x, ymax = mean_x + se_x), alpha = 0.5, fill="darkgreen") +
  ylab("Horizontal coordinates (in px)") +
  xlab("Time (in ms)") +
  ylim(1920, 0) +
  theme_bw() +
...
ggsave(
  gaze_x_plot,
  filename = "graphs/continuity2/continuity2_x-gaze.png",
  height = 5,
  width = 10,
  scale = 0.8
)
...
ia_file <- "data/InterestAreas/IA_continuity_ver2.ias"
interest_areas <- read_eyelink_ia(ia_file)

# Load your image and get dimensions
image_path <- "screenshots/continuity_ver2_control.png"  # Updated image path
...
continuity_ver2_control_photo <- "screenshots/continuity_ver2_control.png"
continuity_ver2_test_photo <- "screenshots/continuity_ver2_test.png"
continuity_ver2_end_photo <- "screenshots/continuity_ver2_end.png"

continuity_ver2_control_photo_gg <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(continuity_ver2_control_photo)
continuity_ver2_test_photo_gg <- ggdraw(theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))) + draw_image(continuity_ver2_test_photo)

continuity_ver2_end_photo_gg <- readPNG(continuity_ver2_end_photo)
```


```{r}
# Create the base ggplot with the image as the background
continuity_ver2_end_photo_gg<- ggplot() +
  # Add the image directly into the ggplot using annotation_custom
  annotation_custom(
    rasterGrob(continuity_ver2_end_photo_gg, width = unit(1, "npc"), height = unit(1, "npc")),
  ...
  theme(
    plot.margin = unit(c(-0.55, -0.55, -0.55, -0.55), "cm"),  # Set negative margins to remove excess space
...
continuity2_pics <- plot_grid(
  continuity_ver2_control_photo_gg,
  continuity_ver2_test_photo_gg,
  continuity_ver2_end_photo_gg,
  nrow = 1,
  labels = c("A - Control", "B - Test", "C - End Position"),
  label_colour = "white"
)
```
...
```{r}
library(cowplot)
pg1<-plot_grid(agg_data_plot, plot_diff_gamm01, continuity2_plot_dwell_end_IP_all_IA, continuity2_plot_dwell_end_IP_IAtransitioncount, rel_widths = c(2,1, 1, 1),labels=c("D", "E", "F", "G"), nrow = 1)

pg2<-plot_grid(continuity2_pics,pg1, ncol=1)

ggsave(pg2, filename = "graphs/continuity2/continuity2_pupil_data.png", width = 15, height=8, scale=0.8)
